...
2. Repository name 填 `didit`，**不要**勾选 "Add a README"（保持空仓库）
3. 建好后，在项目目录执行（把 `你的用户名` 换成你的 GitHub 用户名）：
   ```bash
   git remote add origin https://github.com/你的用户名/didit.git
...
3. 一键建仓并推送：
   ```bash
   gh repo create didit --private --source=. --push
   ```
